...
class CloudStrmBenbear(_PluginBase):
    ...
    # 123云盘相关属性
    _enable_123pan = False
    _pan123_uid = None
    _pan123_private_key = None
    _pan123_expire_minutes = 525600  # 链接有效期，默认一年（525600分钟）
    _pan123_show_uid = True  # 是否在URL中显示UID，默认显示
    ...
    def init_plugin(self, config: dict = None):
        ...
        if config:
            ...
            # 新增：123云盘配置
            self._enable_123pan = config.get("enable_123pan")
            self._pan123_uid = config.get("pan123_uid")
            self._pan123_private_key = config.get("pan123_private_key")
            self._pan123_show_uid = config.get("pan123_show_uid", True)
            # 新增：读取路径替换规则
    ...
    def __format_content(format_str: str, local_file: str, cloud_file: str, uriencode: bool):
        ...
        if "{local_file}" in format_str:
            ...
            # 检查是否是123云盘格式
            if self._enable_123pan and "123pan" in format_str:
                return self.__generate_123pan_url(cloud_file)
            
            # 检查是否包含UID
            if self._pan123_uid in format_str:
                return format_str.replace("{cloud_file}", f"/{self._pan123_uid}{cloud_file}")
            else:
                return format_str.replace("{cloud_file}", cloud_file)
        else:
            return None

    def __generate_123pan_url(self, cloud_file: str) -> str:
        """
        生成123云盘直链URL
        """
        import hashlib
        ...
        # 生成过期时间戳（当前时间 + 有效期）
        timestamp = int(time.time()) + (self._pan123_expire_minutes * 60)
        # 生成随机数 - 使用PHP示例的方式
        rand = random.randint(0, 100000)
        
        # 构建待签名字符串 - 按照PHP示例修改
        # URI格式：/{uid}/path 或 /path（取决于show_uid）
        if self._pan123_show_uid:
            uri = f"/{self._pan123_uid}{cloud_file}"
        else:
            uri = cloud_file
            
        # 签名字符串格式：uri-timestamp-rand-uid-private_key
        sign_str = f"{uri}-{timestamp}-{rand}-{self._pan123_uid}-{self._pan123_private_key}"
        
        # 计算MD5
        md5hash = hashlib.md5(sign_str.encode('utf-8')).hexdigest()
    ...
    def __update_config(self):
        ...
        self.update_config({
            ...
            "enable_123pan": self._enable_123pan,
            "pan123_uid": self._pan123_uid,
            "pan123_private_key": self._pan123_private_key,
            "pan123_show_uid": self._pan123_show_uid,
            "pan123_expire_minutes": self._pan123_expire_minutes,
        })
    ...
    def get_form(self) -> Tuple[List[dict], Dict[str, Any]]:
        ...
        return [
                                ...
                                'content': [
                                    {
                                        'component': 'VTextField',
                                        'props': {
                                            'model': 'pan123_private_key',
                                            'label': '123云盘密钥',
                                            'placeholder': '例如：289ds32418bxdba'
                                        }
                                    }
                                ]
                            }
                                ...
                                'content': [
                                    {
                                        'component': 'VTextField',
                                        'props': {
                                            'model': 'pan123_expire_minutes',
                                            'label': '链接有效期(分钟)',
                                            'placeholder': '例如：525600'
                                        }
                                    }
                                ]
                            }
            ...
            "enable_123pan": False,  # 新增：123云盘配置默认值
            "pan123_uid": "",
            "pan123_private_key": "",
